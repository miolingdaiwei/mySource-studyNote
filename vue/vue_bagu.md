...
proxy相对于difineProperty，直接代理整个对象，包括对象的增加和删除属性。

能够监听数组的变化。

## diff

diff算法就是比较节点，不相同直接替换，相同比较子节点

## key

v-if中，由于会进行比较复用，假如有相同节点复用，但input的value保存，不符合，因此需要key来来来标识，避免复用。

而在v-for中就是为了高效更新渲染dom，实现复用。
